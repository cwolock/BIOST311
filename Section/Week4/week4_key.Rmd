...
\textcolor{blue}{IMR appears to vary quite a bit by region. In particular, median IMR is highest in Africa and Oceania (CHARLIE: It looks like Oceania and Asia are quite similar? I think Asia is actually a bit higher), and the range of IMR is much higher in African than in other regions. Asia and Oceania also have relatively high ranges of IMR. Europe and North America appear to have the lowest median IMR. The North Atlantic region (consisting only of Greenland) did not have a recorded IMR, and so it is excluded from the plot.}
...
\textcolor{blue}{IMR and percent urban appear to be weakly, negatively correlated. You were not expected to compute the correlation here, but of note it is -0.6, which aligns with our guess of weak, negative correlation. (CHARLIE: I wouldn't necessarily call 0.6 weak, maybe moderate?)}
...
8. You should have noticed that your regression slope estimate is a bit unreasonable (CHARLIE: Do you mean it's not on a very useful scale, maybe? I think they might be confused/think you mean "unreasonble" as in "there's something wrong with this result."), in that we are comparing countries that differ in GDP per capita by a single US dollar. Fit a new linear regression model to answer your statistical question in Question 6 that allows you to compare countries that differ in GDP per capita by \$10,000. State the null and alternative hypothesis, and interpret your results. Results should include a point estimate, confidence interval, and p-value.
