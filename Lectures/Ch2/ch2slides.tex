...
\begin{frame}{\includegraphics[scale=0.01]{./figs/chilipepper} Other potential issues}
	...
	A binary variable with mean (probability) of $p$ has variance $p(1-p)$. There is an inherent mean-variance relationship. 
	\\ ~\ 
...
\begin{frame}{Logistic regression: fitting the model in \texttt{R}}
	...
	GLM stands for ``generalized linear model" and represents a whole class of regression methods (more on that later!). For now, all you need to know is that \texttt{glm} syntax is a lot like \texttt{lm} syntax, but we must also specify $\texttt{family = binomial()}$. Our command will look like
	\\ ~\
...
\begin{frame}{Sensitivity and specificity}
	You may remember \textcolor{blue}{sensitivity} and \textcolor{blue}{specificity} from 310, in the context of testing for a disease. We'll be a bit more general here. \pause
	\begin{itemize}
...
\begin{frame}{Sensitivity and specificity}
	How are sensitivity and specificity affected by our choice of cutoff $c$? \pause
	\begin{itemize}
...
\begin{frame}{Logistic regression: Prediction summary}
	\begin{itemize}
		\item Just as with linear regression, we get predictions for logistic regression but plugging covariate values into our regression model with estimated coefficients \pause
...
\begin{frame}{Logistic regression: Adjustment variables}
	...
	\begin{itemize}
		...
		\item Precision variable. Causally related to outcome but not predictor of interest. Including in model increases precision
	\end{itemize}
\end{frame}
...
\begin{frame}{Logistic regression: Adjustment variables}
	...
	The good news is: \textcolor{blue}{The strategy of adjusting for confounders in our model has not changed!} We're just working on log-odds scale now. 
	\\ ~\ 
...
\begin{frame}{Logistic regression: Interaction/effect modification}
	Suppose $W$ is ibinary. Let's look at the difference in log-odds comparing groups differing by one unit in $X$, among those with $W = 0$:
	\begin{small}
	\begin{align*}
...
\begin{frame}{Logistic regression: Precision variables}
	Recall from our linear regression unit that a precision variable $Z$ is 
	\begin{enumerate}
	...
	For this class, all you need to know is that precision variables don't work the same way in logistic regression. In some cases, adjusting for $Z$ in logistic regression can \textit{increase} our standard error on the coefficient for $X$. 
	\\ ~\ 
