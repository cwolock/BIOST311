...
TAYLOR: I know the title says it, but we should maybe mention somewhere on a slide that what we're getting to is cox proportional hazards regression
...
\end{itemize}
Without further assumptions, this is called a \textbf{semiparametric model}. TAYLOR: Maybe just briefly mention what this means? Feels weird to just throw the word in without explaining it
\end{frame}
...
This assumption is violated, for example, if the survival curves cross! TAYLOR: Maybe just include a picture here where survival curves cross so they can see what it looks like
\end{frame}
