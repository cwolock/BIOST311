...
\begin{itemize}
\item Is lung function (\textcolor{blue}{FEV}) associated with smoking? TAYLOR: maybe remove this bullet point cause we didn't actually do this one really
...
\vspace{0.2cm}
TAYLOR: Isn't treating them as missing essentially the same as throwing them out?
Should we treat these incompletely observed times as missing data? Should we throw them out? \pause \textcolor{red}{Either of these is a bad idea!}\pause
...
\begin{frame}
\frametitle{Fraction-at-risk}
TAYLOR: Haven't explained what simulation parameters are prior to this slide
We can plot the fraction-at-risk over time. It won't be a smooth curve, but we can compare it to the expected fraction-at-risk (which we calculate from our simulation parameters).
\begin{center}
...
\item In our panic attack example, $h(t)$ refers to the incidence rate of panic attacks at time $t$ after randomization \pause TAYLOR: Maybe have a reminder of what incidence rate is? Might be useful to have a slide reminding them about prevalence vs. incidence as well
\end{itemize}
...
\begin{frame}
\frametitle{The hazard function} 
TAYLOR: what is the intention behind having this slide here?
\centering
...
\end{center}
TAYLOR: Maybe have an answer slide here also, so they can look back at the notes later
\end{frame}
...
\end{align*}
as an estimator of $S(t)$ (in other words, just count how many people made it past time $t$ without an event). This is a \textbf{nonparametric} estimator; we do not have to postulate a parametric model for the true survival distribution.
TAYLOR: Maybe an example of what it means for something to be parametric, like just even noting that regression coefficients are parameters that we estimate in linear and logistic regression
\\ ~\ 
...
\frametitle{Nonparametric estimation of a survival function}
\vspace{-0.8cm}
Denote by $u_1 < u_2 < \dots < u_m$ the observed ordered, distinct times. (\textbf{Question}: When is $m$ equal to $n$, the sample size? \pause \textbf{Answer}: When all the times are unique (no ties).)\pause 
\\ ~\ TAYLOR: times of what? Do you mean censoring/observation times in your data?
...
\end{align*}
Suppose the data are $\{4, 5, 5+, 8, 12+, 13, 18+, 23, 23, 30\}$.\pause TAYLOR: Maybe include a column for $k$, I would find this easier later on when looking at the table and calculating probabilities if there was a k column
...
\begin{frame}{Conditional and joint probability}
	...
	\begin{align*}
		P(A, B) = P(A \mid B)P(B).
	\end{align*}
	TAYLOR: I have a venn diagram graphic that might make this more intuitive for people, if I end up being the one going over these slides I could draw it myself, otherwise I can make some example in powerpoint. Might be helpful to include idk
\end{frame}
...
\item the KM estimator only jumps at observed event times \pause TAYLOR: Maybe explain what you mean by jumping
...
\begin{itemize}
	...
	TAYLOR: Maybe explain a few steps in the process of filling in this table? I can never keep track of shit with this method of filling things in
\end{itemize}
		\begin{footnotesize}
...
\hline
\end{tabular}
TAYLOR: Do you want to include some comment about the intervals that go outside of 0 and 1 here, just like about being cautious when interpreting them or something?
\end{frame}
...
In practice, we have more than two subgroups. In fact, our predictor might be continuous, so defining subgroups isn't straightforward.
\\ ~\ 
...
\end{align*}
for each $t$, where $h_0$ is an \textcolor{blue}{unspecified\textsuperscript{*}} hazard function.
\\ ~\ 

How does this relate to the hazard ratio. We'll see!
\vfill
\small{\textsuperscript{*}What does this mean? It turns out that to estimate the $\beta$ parameter of this model, we don't ever have to deal with $h_0$.}
\end{frame}
...
\begin{frame}{Cox proportional hazards regression in \texttt{R}}
	...
	\begin{enumerate}
		\item We use the function \texttt{coxph}
...
\begin{frame}{Cox proportional hazards regression: adjustment variables}
	...
	Finally, as in logistic regression, \textcolor{blue}{effect modification} is assessed using an interaction term that is interpreted as a ratio of ratios.
\end{frame}
...
Aside from requiring independent censoring, the proportional hazards model makes another (potentially strong) assumption.
\\ ~\ 
...
\begin{frame}{Cox proportional hazards regression: assumptions}
	...
	There are ways to assess proportional hazards using generalizations of the residual-vs.-fitted plots we saw in Chapter 1, although we won't get into them. For this course, just be aware of how strong this assumption may.
\end{frame}
