...
\item In our panic attack example, $h(t)$ refers to the incidence rate of panic attacks at time $t$ after randomization \pause 
\end{itemize}
...
\end{itemize}
		...
		\begin{tabular}{|c|c|c|c|c|}
			...
			\hline
			4 & 1 & 10 & $\frac{1}{10} = 0.1$ & $9\cdot 0.1= 0.9$\\
...
\begin{frame}{\texttt{mayo} dataset}
	...
	The \texttt{mayo} dataset comes from a \textcolor{blue}{randomized controlled trial} of 
\end{frame}
...
\end{center}
Here, $\hat{S}(t)$ jumps from above 0.5 to below 0.5 without hitting it exactly.
\end{frame}
...
If $H_0$ is true, then we expect that the rows and columns are independent. That means we can compute \textcolor{blue}{expected count} of deaths and compare to the observed counts.
\end{frame}
...
\begin{frame}
\frametitle{Cox proportional hazards regression in \texttt{R}}
...
\texttt{coxph(Surv(obstime, event) $\sim$ X\_1 + \dots)}
\begin{itemize}
...
When adjusting for \textcolor{green}{precision variables}, as in logistic regression, \textcolor{green}{the estimate in the adjusted regression analysis will be further from one} than the unadjusted estimate.
