...
We can aggregate $o(t) - e(t)$ over \textcolor{orange}{all distinct observed failure times} to get the \textbf{logrank test statistic}
\begin{align*}
...
\begin{frame}
\frametitle{The hazard ratio}
The hazard ratio at time $t$ tells us \textcolor{blue}{how much more likely it is that an event will happen in subgroup 1 at time $t$ versus in subgroup 0 at time $t$ \textit{given that it has not yet happened in either subgroup.}}
\begin{align*}
...
\begin{frame}
\frametitle{Cox proportional hazards regression}
An equivalent, and not uncommon (though slightly less precise on what the hazard \textit{means}) interpretation is: \textcolor{blue}{$e^{\beta_1}$ is the hazard ratio comparing two groups differing by one unit in $X_1$ but with the same values of $X_2, \dots, X_p$.}
